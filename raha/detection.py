...
import raha.tools as tools
from raha.tools.KATARA import katara
import numpy as np
#######################################
...
class Detection:
    ...
    def __init__(self):
        ...
        self.ERROR_DETECTION_ALGORITHMS = ["OD", "PVD","RVD","SD","KBVD"]
        ...
        if algorithm == "OD":
            ...
            for i, row in d.dataframe.iterrows():
                if row[l_attribute]:
                    if row[l_attribute] not in value_dictionary:
                        value_dictionary[row[l_attribute]] = {}
                    if row[r_attribute] not in value_dictionary[row[l_attribute]]:
                        value_dictionary[row[l_attribute]][row[r_attribute]] = 1
                    else:
                        value_dictionary[row[l_attribute]][row[r_attribute]] += 1
            sta_dependency = {}
            for key in value_dictionary:
                x = sorted(value_dictionary[key].items(), key=lambda item: item[1])
                sta_dependency[key] = x[-1][0]

            for i, row in d.dataframe.iterrows():
                if row[l_attribute] in value_dictionary:
                    if row[r_attribute] != sta_dependency[row[l_attribute]]:
                        outputted_cells[(i, l_j)] = ""
                        outputted_cells[(i, r_j)] = ""
        elif algorithm == "KBVD":
            outputted_cells = katara.run(d, configuration)
    ...
    def predict_labels(self, d):
        ...
        for j in range(d.dataframe.shape[1]):
            ...
            if sum(y_train) == len(y_train):
                predicted_labels = {i: 1 for i in range(d.dataframe.shape[0])}
            elif sum(y_train) == 0 or len(x_train[0]) == 0:
                predicted_labels = {i: 0 for i in range(d.dataframe.shape[0])}
            else:
                ...
                pro = classification_model.predict_proba(x_test)
                predicted_labels = {}
                ...
                # print(classification_model.predict(x_test))
            #for i, pl in enumerate(predicted_labels):
            for k in predicted_labels:
                # print("k:{}".format(k))
                # print(type(k))
                pl = predicted_labels[k]
                if (k in d.labeled_tuples and d.extended_labeled_cells[(k, j)]) or (k not in d.labeled_tuples and pl):
                    detected_cells_dictionary[(k, j)] = 1
                elif (k in d.labeled_tuples and not d.extended_labeled_cells[(k, j)]) or (k not in d.labeled_tuples and not pl):
                    detected_cells_dictionary[(k, j)] = 0
            if self.VERBOSE:
    ...
    def run(self, dd):
        ...
        shutil.rmtree(d.results_folder)
...
if __name__ == "__main__":
    ...
    print(dataset_dictionary["path"])
    print(dataset_dictionary["clean_path"])

    # dataset_name = 'merge_sample_data'
    # dataset_dictionary = {
    #     "name": dataset_name,
    #     "path": "/home/zju/gyx/feddetect/datasets/distri_datasets/AL_DA5/2/merge_sample_data.csv",
    #     "clean_path": "/home/zju/gyx/feddetect/datasets/distri_datasets/AL_DA5/2/merge_ground_truth.csv"
    # }
    ...
    print("Raha's avg_performance on {}:\nPrecision = {:.2f}\nRecall = {:.2f}\nF1 = {:.2f}".format(data.name, p_avg, r_avg, f_avg))
